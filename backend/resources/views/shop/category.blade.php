<x-public-layout title="{{ $category->name }} - Cielo Carnes">
    <!-- Hero Section -->
    <section class="bg-gradient-to-r from-red-600 to-red-800 text-white py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center">
                ...
                @else
                    <p class="text-xl md:text-2xl mb-8">Descubre nuestros productos de {{ strtolower($category->name) }}</p>
                @endif
            </div>
        </div>
    </section>
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-1/4">
                <div class="bg-white rounded-lg shadow-md p-6 sticky top-4">
                    ...
                    <!-- Búsqueda -->
                    <form method="GET" action="{{ route('shop.category', $category) }}" class="mb-6">
                        <div class="relative">
                            <input type="text"
                                   name="search"
                    ...
                    @if($category->parent)
                        ...
                        <div class="mb-6">
                            ...
                            <div class="space-y-2">
                                <a href="{{ route('shop.category', $category->parent) }}"
                                   class="block px-3 py-2 text-sm text-gray-600 hover:text-red-600">
                                    ← Volver a {{ $category->parent->name }}
                                </a>
                                @foreach($category->parent->children as $sibling)
                                    <a href="{{ route('shop.category', $sibling) }}"
                                       class="block px-3 py-2 text-sm {{ $sibling->id == $category->id ? 'bg-red-100 text-red-700 rounded-md' : 'text-gray-600 hover:text-red-600' }}">
                                        {{ $sibling->name }}
                                    </a>
                                @endforeach
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="space-y-2">
                                    @foreach($category->children as $subcategory)
                                        <a href="{{ route('shop.category', $subcategory) }}"
                                           class="block px-3 py-2 text-sm text-gray-600 hover:text-red-600">
                                            {{ $subcategory->name }}
                                        </a>
                                    @endforeach
            ...
            <div class="lg:w-3/4">
                ...
                <form id="filters-form" method="GET" action="{{ route('shop.category', $category) }}" class="hidden">
                ...
                @else
                    ...
                    <div class="bg-white rounded-lg shadow-md p-8 text-center">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 justify-center">
                            <a href="{{ route('shop.category', $category) }}" class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-md font-medium transition-colors">
                                Limpiar filtros
                            </a>
                            <a href="{{ route('shop.index') }}" class="border border-gray-300 text-gray-700 hover:bg-gray-50 px-6 py-2 rounded-md font-medium transition-colors">
